...
export const getStaticProps = async (context: any) => {
  ...
  const commerceContext = await getCommerceContext(preview, lang, lang, Array.isArray(slug) ? slug.at(0) : slug);

  context.language = lang;
  context.country = country;
  context.market = '13268';
...
export const getStaticPaths = async () => {
  ...
  let paths: string[] = ['/en/products/pdp'];
  ...
  countries.forEach(country => {
    ...
    languages.forEach((language: string) => {
      ...
      paths = paths.concat(localizedSlugs?.map((slug: string) => `/${language}/products/${slug}`));
    });
  });
